...
class _GoalStat extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Expanded(child: Container(
    ...
    child: Column(children: [
      Text(value, style: TextStyle(fontFamily: 'Roboto', fontSize: 22, fontWeight: FontWeight.w700, color: color)),
      Text(label, style: const TextStyle(fontFamily: 'Roboto', fontSize: 9, color: AppColors.textSecondary)),
    ]),
  ));
}
...
class _SectionHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Row(children: [
      ...
      Padding(padding: const EdgeInsets.symmetric(horizontal: 10), child: Text(label, style: const TextStyle(fontFamily: 'Roboto', fontSize: 9, color: AppColors.textSecondary, letterSpacing: 1.5))),
      Expanded(child: Divider(color: AppColors.border.withValues(alpha: 0.6))),
    ]),
  );
}
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Row(children: [
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Priority', style: TextStyle(fontSize: 10, color: AppColors.textSecondary, fontFamily: 'Roboto')),
          const Gap(5),
            ...
            return Expanded(child: Padding(
              ...
              child: GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  child: Center(child: Text(p, style: TextStyle(fontFamily: 'Roboto', fontSize: 10, color: sel ? c : AppColors.textSecondary, fontWeight: sel ? FontWeight.w700 : FontWeight.w400))),
                ),
              ),
            ));
          }).toList()),
        ...
        const Gap(10),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Target Date *', style: TextStyle(fontSize: 10, color: AppColors.textSecondary, fontFamily: 'Roboto')),
          const Gap(5),
          GestureDetector(
            onTap: onDateTap,
            child: Container(
              ...
              child: Row(children: [
                ...
                const Gap(6),
                Text(DateFormat('dd MMM yy').format(target), style: const TextStyle(fontFamily: 'Roboto', fontSize: 11, color: AppColors.textPrimary)),
              ]),
            ),
          ),
        ])),
      ...
      const Gap(10),
      Row(children: [
        const Text('Starting progress:', style: TextStyle(fontSize: 10, color: AppColors.textSecondary, fontFamily: 'Roboto')),
        Expanded(child: Slider(value: progress.toDouble(), min: 0, max: 100, divisions: 20, onChanged: (v) => onProgressChange(v.round()))),
        Text('$progress%', style: const TextStyle(fontFamily: 'Roboto', fontSize: 11, color: AppColors.gold)),
      ]),
      Row(children: [
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            const Gap(6),
            Text(goal.priority.toUpperCase(), style: TextStyle(fontFamily: 'Roboto', fontSize: 8, color: priColor, fontWeight: FontWeight.w700, letterSpacing: 0.5)),
            if (goal.status == 'done') ...[
              const Gap(8),
              const Text('COMPLETE ✓', style: TextStyle(fontFamily: 'Roboto', fontSize: 8, color: AppColors.deen, fontWeight: FontWeight.w600)),
            ],
            const Spacer(),
            Text(daysLeft < 0 ? 'Overdue' : daysLeft == 0 ? 'Today!' : '${daysLeft}d left', style: TextStyle(fontFamily: 'Roboto', fontSize: 9, color: daysColor)),
          ]),
          const Gap(6),
          ...
          Row(children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                const Text('Progress', style: TextStyle(fontFamily: 'Roboto', fontSize: 9, color: AppColors.textSecondary)),
                const Spacer(),
                Text('${goal.progress}%', style: TextStyle(fontFamily: 'Roboto', fontSize: 10, color: priColor, fontWeight: FontWeight.w600)),
              ]),
              const Gap(4),
              SliderTheme(
            ...
            if (_addingSubtask)
              ...
              GestureDetector(
                ...
                child: Row(children: const [
                  ...
                  Gap(4),
                  Text('Add subtask', style: TextStyle(fontFamily: 'Roboto', fontSize: 9, color: AppColors.textSecondary)),
                ]),
              ),
            const Gap(6),
          ],
          ...
          const Gap(2),
          Row(children: [
            Text(DateFormat('d MMM yy').format(goal.targetDate), style: const TextStyle(fontFamily: 'Roboto', fontSize: 9, color: AppColors.textSecondary)),
            const Spacer(),
...
class _StatusBtn extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(
      ...
      child: Text(label, style: TextStyle(fontFamily: 'Roboto', fontSize: 9.5, color: color, fontWeight: FontWeight.w600)),
    ),
  );
}