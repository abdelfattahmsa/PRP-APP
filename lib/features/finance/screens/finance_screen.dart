        ...
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              const Text('Total Debt', style: TextStyle(fontFamily: 'Roboto', fontSize: 10, color: AppColors.textSecondary, letterSpacing: 1)),
              Container(
                ...
                child: const Text('TARGET ≤ 100K · SEP 2026', style: TextStyle(fontFamily: 'Roboto', fontSize: 8, color: AppColors.error, fontWeight: FontWeight.w600)),
              ),
            ]),
            const Gap(8),
            Text(egp(summary.totalDebt), style: const TextStyle(fontFamily: 'Roboto', fontSize: 28, fontWeight: FontWeight.w700, color: AppColors.error)),
            const Gap(12),
            ClipRRect(
            ...
            const Gap(6),
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text(egp(summary.totalDebt), style: const TextStyle(fontFamily: 'Roboto', fontSize: 10, color: AppColors.textSecondary)),
              Text('${((1 - debtProgress) * 100).toStringAsFixed(0)}% to target', style: const TextStyle(fontFamily: 'Roboto', fontSize: 10, color: AppColors.textSecondary)),
              const Text('100K', style: TextStyle(fontFamily: 'Roboto', fontSize: 10, color: AppColors.textSecondary)),
            ]),
          ]),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
        Text(label, style: const TextStyle(fontFamily: 'Roboto', fontSize: 9, color: AppColors.textSecondary, letterSpacing: 0.5)),
        const Gap(4),
        Text(value, style: TextStyle(fontFamily: 'Roboto', fontSize: 14, fontWeight: FontWeight.w700, color: color)),
      ]),
    );
  }
}
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (!_editing && widget.bank.creditCardLimit > 0) ...[
          ...
          const Gap(4),
          Text('Limit: ${egp(widget.bank.creditCardLimit)}', style: const TextStyle(fontFamily: 'Roboto', fontSize: 9, color: AppColors.textSecondary)),
        ],
      ]),
...
class _NumField extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Text(label, style: const TextStyle(fontSize: 9, color: AppColors.textSecondary, fontFamily: 'Roboto')),
    const Gap(3),
    TextField(controller: ctrl, keyboardType: TextInputType.number, textAlign: TextAlign.right,
        style: const TextStyle(fontFamily: 'Roboto', fontSize: 12, color: AppColors.textPrimary),
...
class _BankStat extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Text(label, style: const TextStyle(fontFamily: 'Roboto', fontSize: 8.5, color: AppColors.textSecondary)),
    const Gap(3),
    Text(value, style: TextStyle(fontFamily: 'Roboto', fontSize: 11, fontWeight: FontWeight.w600, color: color)),
  ]);
}
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        _editing
            ? SizedBox(width: 100, child: TextField(controller: _ctrl, keyboardType: TextInputType.number, textAlign: TextAlign.right, autofocus: true, style: const TextStyle(fontFamily: 'Roboto', fontSize: 13, color: AppColors.textPrimary), decoration: const InputDecoration(isDense: true, contentPadding: EdgeInsets.symmetric(horizontal: 10, vertical: 6)), onSubmitted: (_) => _save()))
            : Text('EGP ${_fmt.format(cash)}', style: const TextStyle(fontFamily: 'Roboto', fontSize: 13, color: AppColors.gold, fontWeight: FontWeight.w600)),
        const Gap(8),
        IconButton(
          ...
          children: [
            if (debts.isNotEmpty) Container(
              ...
              child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                ...
                Text(egp(total), style: const TextStyle(fontFamily: 'Roboto', fontSize: 16, color: AppColors.error, fontWeight: FontWeight.w700)),
              ]),
            ),
            const Gap(10),
...
class _DebtTile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) => Container(
    ...
    child: Row(children: [
      ...
      Text(egp(debt.amount), style: const TextStyle(fontFamily: 'Roboto', fontSize: 14, color: AppColors.error, fontWeight: FontWeight.w600)),
      const Gap(8),
              ...
              children: ['All', ...AppConstants.txCategories].map((c) {
                ...
                return Padding(
                  ...
                  child: FilterChip(
                    ...
                    labelStyle: TextStyle(fontFamily: 'Roboto', fontSize: 10, color: sel ? AppColors.gold : AppColors.textSecondary, fontWeight: sel ? FontWeight.w600 : FontWeight.w400),
            ...
            child: Row(children: [
              ...
              if (filtered.isNotEmpty) ...[
                ...
                Container(
                  ...
                  child: Text(egp(total), style: const TextStyle(fontFamily: 'Roboto', fontSize: 12, color: AppColors.error, fontWeight: FontWeight.w600)),
                ),
              ],
            ]),
          ),
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(children: [
      ...
      Row(children: [
        ...
        const Gap(10),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Account', style: TextStyle(fontSize: 10, color: AppColors.textSecondary, fontFamily: 'Roboto')),
          const Gap(5),
          DropdownButtonFormField<String>(
            initialValue: acct, onChanged: (v) => onAcctChange(v!),
            dropdownColor: AppColors.card,
            items: accounts.map((a) => DropdownMenuItem(value: a, child: Text(a, style: const TextStyle(fontSize: 12, fontFamily: 'Roboto')))).toList(),
      ...
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Category', style: TextStyle(fontSize: 10, color: AppColors.textSecondary, fontFamily: 'Roboto')),
        const Gap(5),
        DropdownButtonFormField<String>(
          initialValue: cat, onChanged: (v) => onCatChange(v!), dropdownColor: AppColors.card,
          items: AppConstants.txCategories.map((c) => DropdownMenuItem(value: c, child: Text(c, style: const TextStyle(fontSize: 12, fontFamily: 'Roboto')))).toList(),
          decoration: const InputDecoration(contentPadding: EdgeInsets.symmetric(horizontal: 10, vertical: 8), isDense: true),
        ),
      ]),
      const Gap(10),
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text('${tx.accountName} · ${tx.category} · ${DateFormat('dd MMM').format(tx.date)}', style: const TextStyle(fontFamily: 'Roboto', fontSize: 9.5, color: AppColors.textSecondary)),
        ])),
        Text('${tx.isIncome ? '+' : '-'}${egp(tx.amount)}', style: TextStyle(fontFamily: 'Roboto', fontSize: 13, color: tx.isIncome ? AppColors.deen : AppColors.error, fontWeight: FontWeight.w600)),
        const Gap(6),
